//! Domain policy for Department aggregate
//!
...
/// Domain policy for Department — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Department>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type DepartmentDomainPolicy = PermitAllPolicy<Department>;
